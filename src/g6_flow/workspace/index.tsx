...
export const Workspace = () => {
  ...
  useEffect(() => {
    if (!graph) {
      ...
      if (miniMapRef.current != null) {
        minimap = new G6.Minimap({
          size: [
            (miniMapRef.current as HTMLDivElement).clientWidth,
            (miniMapRef.current as HTMLDivElement).clientHeight,
          ],
    ...
    graph.on("node:mouseenter", (evt) => {
      if (evt.item != null) graph.setItemState(evt.item, "hover", true);
    });

    graph.on("node:mouseleave", (evt) => {
      if (evt.item != null) graph.setItemState(evt.item, "hover", false);
    });
